...
class Net(nn.Module):
    def __init__(self,num_classes=1,input_channels=3, block=Res_block, num_blocks=[3, 4, 6, 3], nb_filter=[16, 32, 64, 128, 256], deep_supervision=False):
        ...
        self.Upsample = nn.Sequential(
            nn.Conv2d(16, 256, kernel_size=8, stride=8),
            nn.Conv2d(256,256,1,1)
        )
        self.head = SAHead(num_calsses=1)
    ...
    def forward(self, input):
        ...
        if self.deep_supervision:
            ...
            #print(Final_x0_4.shape)
            x = self.Upsample(Final_x0_4)
            #print(x.shape)
            return self.head(x)
