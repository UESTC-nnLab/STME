...
class YOLOLoss(nn.Module):    
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        #-------------------------------------------------------#
        max_dynamic_k = min(50, fg_mask.sum().item())  # 设置一个合理的最大动态K值，根据硬件资源和数据特性调整
    ...
    #     n_candidate_k           = min(10, pair_wise_ious.size(1))
    #     topk_ious, _            = torch.topk(pair_wise_ious, n_candidate_k, dim=1)
    #     dynamic_ks              = torch.clamp(topk_ious.sum(1).int(), min=1, max=max_dynamic_k)
    ...
    #     # 逐行累加anchor_matching_gt，避免一次性创建大型张量
    #     anchor_matching_gt = torch.zeros(fg_mask.shape[0])
    #     for gt_idx in range(num_gt):
    #         '''解决维度不匹配问题'''
    #         # _, pos_idx = torch.topk(cost[gt_idx], k=dynamic_ks[gt_idx].item(), largest=False)
    #         # matching_matrix[gt_idx][pos_idx] = 1.0
    #         ''''''
    #         anchor_matching_gt += matching_matrix[gt_idx]
    ...
    #         #   选取cost最小的真实框。
    #         #------------------------------------------------------------#
    #         problematic_indices = torch.arange(len(anchor_matching_gt))[anchor_matching_gt > 1]
    #         problematic_costs = cost[:, problematic_indices]
            
    #         # 分批处理，避免一次性创建大型张量
    #         for batch in torch.split(problematic_costs, min(10000, problematic_costs.shape[1])):
    #             _, cost_argmin_batch = torch.min(batch, dim=0)
    #             matching_matrix[:, cost_argmin_batch] *= 0.0
